...
when_to_use: Use when determining the level of verification required for a current development task, or when applying zero-gate diagnostic logging during debugging sessions.
...
- **Verification/Bypass Rules**: Completely bypass test writing (no BDD required), linter rules, formatting checks, and file quality-gate rules (e.g. ignore maximum line counts or style enforcement). Never perform secondary cleanups, file refactoring, or split logic when adding temporary debug logging.
...
- **Workflow**: BDD (Targeted Test) -> Code -> Targeted Verification.
