...
when_to_use: Use upon successful completion and verification (via npm run check) of a Tier 2 or Tier 3 development task, or when the user explicitly requests an assessment or post-mortem of completed work.
...
- Create or update dedicated skill folders under `.claude/skills/` (e.g., `.claude/skills/<skill-name>/SKILL.md`).
- Ensure every `SKILL.md` contains fully compliant YAML frontmatter (`description`, `when_to_use`) matching the directory structure for progressive disclosure ingestion.
