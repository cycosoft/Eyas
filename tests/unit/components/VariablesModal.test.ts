...
type ComponentVM = {
	...
	linkIsValid: boolean;
	form: unknown[];
	variables: { type: string, field?: string, options?: unknown[] }[];
...
describe(`VariablesModal`, () => {
	...
	test(`receives link via IPC and displays it`, async () => {
		...
		if (receiveCallback) {
			...
			expect((wrapper.vm as unknown as ComponentVM).link).toBe(testLink);
	...
	test(`validates URLs correctly - valid URL enables launch button`, async () => {
		(wrapper.vm as unknown as ComponentVM).link = `https://example.com`;
		await (wrapper.vm as unknown as ComponentVM).$nextTick();

		// parsedLink should be the same as link when no variables
		expect((wrapper.vm as unknown as ComponentVM).parsedLink).toBe(`https://example.com`);
		expect((wrapper.vm as unknown as ComponentVM).linkIsValid).toBe(true);
	});

	test(`validates URLs correctly - invalid URL disables launch button`, async () => {
		(wrapper.vm as unknown as ComponentVM).link = `not-a-valid-url`;
		await (wrapper.vm as unknown as ComponentVM).$nextTick();

		expect((wrapper.vm as unknown as ComponentVM).parsedLink).toBe(`not-a-valid-url`);
		expect((wrapper.vm as unknown as ComponentVM).linkIsValid).toBe(false);
	});

	test(`validates URLs correctly - URL with variables disables until filled`, async () => {
		(wrapper.vm as unknown as ComponentVM).link = `https://example.com?id={int}`;
		await (wrapper.vm as unknown as ComponentVM).$nextTick();
		...
		(wrapper.vm as unknown as ComponentVM).form = [`123`];
		await (wrapper.vm as unknown as ComponentVM).$nextTick();

		// Should now be valid
		expect((wrapper.vm as unknown as ComponentVM).parsedLink).toBe(`https://example.com?id=123`);
		expect((wrapper.vm as unknown as ComponentVM).linkIsValid).toBe(true);
	});
	...
	test(`variable replacement in parsedLink computed property`, async () => {
		(wrapper.vm as unknown as ComponentVM).link = `https://example.com?msg={str}&id={int}`;
		(wrapper.vm as unknown as ComponentVM).form = [`hello`, `456`];
		await (wrapper.vm as unknown as ComponentVM).$nextTick();

		expect((wrapper.vm as unknown as ComponentVM).parsedLink).toBe(`https://example.com?msg=hello&id=456`);
	});
	...
	test(`form inputs update parsedLink correctly`, async () => {
		...
		expect((wrapper.vm as unknown as ComponentVM).parsedLink).toContain(`{int}`);

		// Update form
		(wrapper.vm as unknown as ComponentVM).form = [`789`];
		await (wrapper.vm as unknown as ComponentVM).$nextTick();

		expect((wrapper.vm as unknown as ComponentVM).parsedLink).toBe(`https://example.com?id=789`);
	});
	...
	describe(`variables computed — type detection`, () => {
		test(`{int} variable is detected with type "int"`, () => {
			(wrapper.vm as unknown as ComponentVM).link = `https://example.com?id={int}`;
			const vars = (wrapper.vm as unknown as ComponentVM).variables;
			expect(vars).toHaveLength(1);
			expect(vars[0].type).toBe(`int`);
		});

		test(`{str} variable is detected with type "str"`, () => {
			(wrapper.vm as unknown as ComponentVM).link = `https://example.com?msg={str}`;
			const vars = (wrapper.vm as unknown as ComponentVM).variables;
			expect(vars).toHaveLength(1);
		...
		test(`{bool} variable is detected with type "bool"`, () => {
			(wrapper.vm as unknown as ComponentVM).link = `https://example.com?enabled={bool}`;
			const vars = (wrapper.vm as unknown as ComponentVM).variables;
			expect(vars).toHaveLength(1);
		...
		test(`{opt1|opt2|} list variable is detected with type "list"`, () => {
			(wrapper.vm as unknown as ComponentVM).link = `https://example.com?env={dev.|staging.|}`;
			const vars = (wrapper.vm as unknown as ComponentVM).variables;
			expect(vars).toHaveLength(1);
		...
		test(`multiple variables in one URL produce the correct count and types`, () => {
			(wrapper.vm as unknown as ComponentVM).link = `https://example.com?id={int}&msg={str}&enabled={bool}`;
			const vars = (wrapper.vm as unknown as ComponentVM).variables;
	...
	describe(`variables computed — field label extraction`, () => {
		...
		test(`query-param variable with string type extracts field "message"`, () => {
			(wrapper.vm as unknown as ComponentVM).link = `https://example.com?message={str}`;
			const vars = (wrapper.vm as unknown as ComponentVM).variables;
		...
		test(`non-query-param variable has no field property`, () => {
			...
			(wrapper.vm as unknown as ComponentVM).link = `https://example.com/{int}/page`;
			const vars = (wrapper.vm as unknown as ComponentVM).variables;
	...
	describe(`getFieldLabel method`, () => {
		test(`returns prefix only when no field is provided`, () => {
			expect((wrapper.vm as unknown as ComponentVM).getFieldLabel(`Enter number`, ``)).toBe(`Enter number`);
			expect((wrapper.vm as unknown as ComponentVM).getFieldLabel(`Enter number`, undefined)).toBe(`Enter number`);
		});

		test(`returns prefix with field name when field is provided`, () => {
			expect((wrapper.vm as unknown as ComponentVM).getFieldLabel(`Enter number`, `id`)).toBe(`Enter number for "id" field`);
			expect((wrapper.vm as unknown as ComponentVM).getFieldLabel(`Select value`, `enabled`)).toBe(`Select value for "enabled" field`);
		});
	});
	...
	describe(`bool and list variable link validity`, () => {
		...
		test(`{bool} URL becomes valid when "true" is selected`, async () => {
			(wrapper.vm as unknown as ComponentVM).link = `https://example.com?enabled={bool}`;
			(wrapper.vm as unknown as ComponentVM).form = [`true`];
			await (wrapper.vm as unknown as ComponentVM).$nextTick();
			expect((wrapper.vm as unknown as ComponentVM).parsedLink).toBe(`https://example.com?enabled=true`);
			expect((wrapper.vm as unknown as ComponentVM).linkIsValid).toBe(true);
		});

		test(`{bool} URL becomes valid when "false" is selected`, async () => {
			(wrapper.vm as unknown as ComponentVM).link = `https://example.com?enabled={bool}`;
			(wrapper.vm as unknown as ComponentVM).form = [`false`];
			await (wrapper.vm as unknown as ComponentVM).$nextTick();
			expect((wrapper.vm as unknown as ComponentVM).parsedLink).toBe(`https://example.com?enabled=false`);
			expect((wrapper.vm as unknown as ComponentVM).linkIsValid).toBe(true);
		});

		test(`list variable URL is invalid until an option is selected`, async () => {
			(wrapper.vm as unknown as ComponentVM).link = `https://{dev.|staging.|}cycosoft.com`;
			await (wrapper.vm as unknown as ComponentVM).$nextTick();
			expect((wrapper.vm as unknown as ComponentVM).linkIsValid).toBe(false);
		});
		...
		test(`list variable URL becomes valid when an option is selected`, async () => {
			(wrapper.vm as unknown as ComponentVM).link = `https://{dev.|staging.|}cycosoft.com`;
			(wrapper.vm as unknown as ComponentVM).form = [`dev.`];
			await (wrapper.vm as unknown as ComponentVM).$nextTick();
			expect((wrapper.vm as unknown as ComponentVM).parsedLink).toBe(`https://dev.cycosoft.com`);
			expect((wrapper.vm as unknown as ComponentVM).linkIsValid).toBe(true);
		});

		test(`list variable URL becomes valid when blank option is selected`, async () => {
			(wrapper.vm as unknown as ComponentVM).link = `https://{dev.|staging.|}cycosoft.com`;
			(wrapper.vm as unknown as ComponentVM).form = [``];
			await (wrapper.vm as unknown as ComponentVM).$nextTick();
			expect((wrapper.vm as unknown as ComponentVM).parsedLink).toBe(`https://cycosoft.com`);
			expect((wrapper.vm as unknown as ComponentVM).linkIsValid).toBe(true);
		});
	});
	...
	describe(`testdomain variable behavior`, () => {
		test(`{testdomain} is NOT in the variables list when pre-resolved before modal opens`, () => {
			...
			(wrapper.vm as unknown as ComponentVM).link = `https://staging.example.com?enabled={bool}`;
			const vars = (wrapper.vm as unknown as ComponentVM).variables;
		...
		test(`{testdomain} only — pre-resolved URL with no other variables is immediately valid`, async () => {
			...
			(wrapper.vm as unknown as ComponentVM).link = `https://staging.example.com?go`;
			await (wrapper.vm as unknown as ComponentVM).$nextTick();
			expect((wrapper.vm as unknown as ComponentVM).linkIsValid).toBe(true);
		});
		...
		test(`if {testdomain} somehow reaches the modal it is treated as a generic variable token`, () => {
			...
			(wrapper.vm as unknown as ComponentVM).link = `https://example.com/{testdomain}/path`;
			const vars = (wrapper.vm as unknown as ComponentVM).variables;
			expect(vars).toHaveLength(1);
	...
	describe(`parsedLink URI encoding`, () => {
		test(`spaces in string values are percent-encoded`, async () => {
			(wrapper.vm as unknown as ComponentVM).link = `https://example.com?msg={str}`;
			(wrapper.vm as unknown as ComponentVM).form = [`hello world`];
			await (wrapper.vm as unknown as ComponentVM).$nextTick();
			expect((wrapper.vm as unknown as ComponentVM).parsedLink).toBe(`https://example.com?msg=hello%20world`);
		});

		test(`special characters in string values are percent-encoded`, async () => {
			(wrapper.vm as unknown as ComponentVM).link = `https://example.com?q={str}`;
			(wrapper.vm as unknown as ComponentVM).form = [`foo&bar`];
			await (wrapper.vm as unknown as ComponentVM).$nextTick();
			expect((wrapper.vm as unknown as ComponentVM).parsedLink).toBe(`https://example.com?q=foo%26bar`);
		});
		...
		test(`empty string value replaces the variable with nothing`, async () => {
			...
			expect((wrapper.vm as unknown as ComponentVM).parsedLink).toBe(`https://example.com?q=`);
		});
	});
	...
	describe(`underscore-prefix (_env) variable behavior`, () => {
		test(`{_env.url} pre-resolved URL has no variables and is immediately valid`, async () => {
			...
			(wrapper.vm as unknown as ComponentVM).link = `https://staging.eyas.cycosoft.com?go`;
			await (wrapper.vm as unknown as ComponentVM).$nextTick();
			expect((wrapper.vm as unknown as ComponentVM).variables).toHaveLength(0);
			expect((wrapper.vm as unknown as ComponentVM).linkIsValid).toBe(true);
		});

		test(`{_env.key} pre-resolved URL has no variables and is immediately valid`, async () => {
			(wrapper.vm as unknown as ComponentVM).link = `https://staging.cycosoft.com`;
			await (wrapper.vm as unknown as ComponentVM).$nextTick();
			expect((wrapper.vm as unknown as ComponentVM).variables).toHaveLength(0);
			expect((wrapper.vm as unknown as ComponentVM).linkIsValid).toBe(true);
		});
		...
		test(`{_env.url} token reaching the modal is treated as an unknown app token, not user-input`, () => {
			...
			(wrapper.vm as unknown as ComponentVM).link = `https://example.com/{_env.url}/path`;
			const vars = (wrapper.vm as unknown as ComponentVM).variables;
			// Should NOT be in variables list (filtered by underscore prefix)
			expect(vars.every((v: { type: string }) => !v.type.startsWith(`_`))).toBe(true);
		});

		test(`{_env.key} token reaching the modal is treated as an unknown app token, not user-input`, () => {
			(wrapper.vm as unknown as ComponentVM).link = `https://{_env.key}example.com`;
			const vars = (wrapper.vm as unknown as ComponentVM).variables;
			// Should NOT be in variables list (filtered by underscore prefix)
			expect(vars.every((v: { type: string }) => !v.type.startsWith(`_`))).toBe(true);
		});
		...
		test(`after {_env.url} and {_env.key} resolved, only remaining user vars appear in modal`, async () => {
			...
			(wrapper.vm as unknown as ComponentVM).link = `https://staging.eyas.cycosoft.com?env=staging.&enabled={bool}`;
			await (wrapper.vm as unknown as ComponentVM).$nextTick();
			const vars = (wrapper.vm as unknown as ComponentVM).variables;
			expect(vars).toHaveLength(1);
			expect(vars[0].type).toBe(`bool`);
