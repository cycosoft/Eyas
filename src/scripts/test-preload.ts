...
import type { ChannelName, DomainUrl, Username, PasswordPlain, ShouldShow } from "@registry/primitives.js";
...
function createDropdownItem(input: HTMLInputElement, cred: DecryptedCredential): HTMLDivElement {
	...
	`);
	const masked = `•••••••`;
	item.innerHTML = `<span class="username">${cred.username}</span><span class="password-mask" style="color:#888;">${masked}</span>`;
...
function showAutocompleteDropdown(input: HTMLInputElement, credentialsList: DecryptedCredential[]): void {
	...
	credentialsList.forEach(cred => {
		const item = createDropdownItem(input, cred);
