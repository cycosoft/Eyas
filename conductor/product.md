...
- **Extend Sessions:** Provide a seamless way for users to extend their manual testing sessions when the internal server reaches its time limit.
...
- **Expose Test (Resume):** Allows users to serve their test content to the local network for manual review, with a resume option to extend sessions beyond the initial 30-minute limit.
